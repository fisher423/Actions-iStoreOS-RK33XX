...
# 网口配置为旁路由模式，注释下面两个网口模式替换命令后，网口模式会变成主路由模式，不知道什么原因理论应该全部变成旁路由模式的，但对于RK3399 fine3399机型网口模式还是主路由模式，没深度研究过，你们自己测试然后修改吧。
sed -i "s/armsom,p2pro)/armsom,p2pro|\\\\\n	rk3399,fine3399)/g" target/linux/rockchip/armv8/base-files/etc/board.d/02_network
sed -i "s/rk3399,fine3399)/rk3399,fine3399|\\\\\n	rk3399,tpm312)/g" target/linux/rockchip/armv8/base-files/etc/board.d/02_network
